...
var BasicRuntime = class {
  constructor(options = {}) {
    this.pyFunctions = /* @__PURE__ */ new Map();
    this.pyFailures = /* @__PURE__ */ new Map();
    this.pyCircuitOpenUntil = /* @__PURE__ */ new Map();
    this.db = null;
    this.dbInit = null;
    this.dbPath = null;
    this.migrationsDir = null;
    this.migrationsApplied = false;
    this.persistState = /* @__PURE__ */ new Map();
    this.persistEvents = new EventEmitter();
    this.pyModulePath = options.pyModulePath;
    this.pythonBinary = options.pythonBinary ?? process.env.UNISTACK_PYTHON_BIN ?? "python";
    this.pyTimeoutMs = Math.max(100, Number(options.pyTimeoutMs ?? process.env.UNISTACK_PY_TIMEOUT_MS ?? 7e3));
    this.pyFailureThreshold = Math.max(1, Number(options.pyFailureThreshold ?? process.env.UNISTACK_PY_FAILURE_THRESHOLD ?? 3));
    this.pyCooldownMs = Math.max(500, Number(options.pyCooldownMs ?? process.env.UNISTACK_PY_COOLDOWN_MS ?? 3e4));
  ...
  subscribePersist(listener) {
    const wrapped = (key, value) => listener(key, value);
...
async function startServer(createApp, runtime, options) {
...
async function callPythonFunction(modulePath, funcName, args, options) {
  const script = `
