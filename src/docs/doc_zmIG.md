...
Multi-line examples above. See `validate` and parameterized `sql` syntax.

## Middleware
Configure auth/CORS/rate-limit.

```text
middleware:
  auth handler py:check(token) excludes "/public";
  cors origins http://example.com;
  rate-limit max=500 window=10m;
```

Production defaults apply when section absent.

## ws-routes
WebSocket endpoints bound to Python handlers.
...
## env
Declare required environment variables and types. Validated at build.
...
## test
Inline JavaScript assertions run during `npm run test` or CI.
