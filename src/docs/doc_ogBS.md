...
### Additional Phase 3.5 Extensions
- **Full LSP Server** enhancing autocomplete, diagnostics and semantic tokens with ORM awareness
- **UniStd Library** (HTTP, UI, FS) so apps need no external packages
- **UniPack** package manager that precompiles modules for web/native targets
- **UI Engine / Beauty Injector** automatically injects design system and themes
