# UniStack — Write your entire full-stack app in one file.

**english:** You don’t need separate frontend and backend repositories, package managers, or servers. UniStack lets you describe your database, routes, UI, and business logic in a single `.uni` document and it just works.

**français :** Vous n’avez plus besoin de repos séparés front/back, de gestionnaires de paquets ou de serveurs multiples. UniStack vous permet de décrire base de données, routes, interface et logique métier dans un seul fichier `.uni` – ça marche.
...
unistack app "Blog" version 1.0 {
  ...
  routes:
    ...
    POST /api/posts {
      validate title:string:required:min=3, content:string:required;
      return sql("INSERT INTO posts (title, content) VALUES (?, ?)", title, content);
    }
  html-ui:
    <div id="app">
      <h1>My Blog</h1>
...
> **30 seconds:** `npx unistack dev` — that’s it.
