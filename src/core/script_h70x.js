...
`;
    const ast = parseUniFile(src, 'test.uni');
    const state = ast.sections.find(s => s.kind === 'state');
...
`;
    const ast = parseUniFile(src, 'test.uni');
    const html = ast.sections.find(s => s.kind === 'html');
    const textNode = html.blocks[0].nodes.find((n) => n.kind === 'htmlText');
...
function testSqlParamsParsing() {
  ...
  html-ui:
    ...
    const ast = parseUniFile(src, 'test.uni');
    const html = ast.sections.find(s => s.kind === 'html');
    const expr = html.blocks[0].nodes.find((n) => n.kind === 'htmlExpr');
...
async function testMiddlewareAndWsBuild() {
    const src = `unistack app "T" version 1.0 {
  middleware:
    cors: origins=["http://localhost:3000"];
    rateLimit: max=100, window="15m";
    helmet: true;
    auth: exclude=["/login"], handler=py:verify_jwt;
  ...
  ws-routes:
    ...
    const tmpDir = resolve('src', 'tests', 'tmp-middleware-ws');
    await fs.mkdir(tmpDir, { recursive: true });
    const entry = resolve(tmpDir, 'app.uni');
    await fs.writeFile(entry, src, 'utf8');
    await buildUniStack({ entryPath: entry, generatedDir: tmpDir });
    ...
    assert(serverTs.includes("app.use(helmet())"), 'server.ts should include helmet middleware');
    assert(serverTs.includes('app.use(cors({ origin: ["http://localhost:3000"] }))'), 'server.ts should include cors middleware');
    assert(serverTs.includes('rateLimit({ windowMs: 900000, max: 100 })'), 'server.ts should include rate-limit middleware');
    assert(serverTs.includes('const authMiddleware ='), 'server.ts should include auth middleware');
    assert(serverTs.includes('new WebSocketServer({ noServer: true })'), 'server.ts should include WebSocketServer setup');
    assert(serverTs.includes("server.on('upgrade'"), 'server.ts should handle WebSocket upgrades');
    assert(clientTs.includes('window.UniSocket'), 'client.ts should include UniSocket helper');

    await fs.rm(tmpDir, { recursive: true, force: true });
    console.log('english: middleware and ws build test passed | french: test build middleware et ws réussi');
}
async function testFileUploadBuild() {
    const src = `unistack app "T" version 1.0 {
  routes:
    POST /upload {
      file: field="avatar", accept=["image/png"], maxSize="5mb";
      return py:handle_upload(file.path, file.name, file.size);
    }
}`;
    const tmpDir = resolve('src', 'tests', 'tmp-upload');
    await fs.mkdir(tmpDir, { recursive: true });
    const entry = resolve(tmpDir, 'app.uni');
    await fs.writeFile(entry, src, 'utf8');
    await buildUniStack({ entryPath: entry, generatedDir: tmpDir });
    const serverTs = await fs.readFile(resolve(tmpDir, 'app.server.ts'), 'utf8');
    assert(serverTs.includes("import multer from 'multer'"), 'server.ts should import multer');
    assert(serverTs.includes("dest: 'uploads/'"), 'server.ts should configure multer destination');
    assert(serverTs.includes("limits: { fileSize: 5242880 }"), 'server.ts should configure file size limit');
    assert(serverTs.includes('fileFilter:'), 'server.ts should include fileFilter for MIME types');
    assert(serverTs.includes("app.post('/upload', multer"), 'server.ts should apply multer middleware to route');
    assert(serverTs.includes("runtime.callPy('handle_upload', (req as any).file?.path, (req as any).file?.originalname, (req as any).file?.size)"), 'server.ts should pass file properties to handler');
    assert(serverTs.includes("fs.unlink((req as any).file.path,"), 'server.ts should include temp file cleanup');
    await fs.rm(tmpDir, { recursive: true, force: true });
    console.log('english: file upload build test passed | french: test build upload de fichier réussi');
}

async function testModuleImports() {
    const tmpDir = resolve('src', 'tests', 'tmp-modules');
    await fs.mkdir(tmpDir, { recursive: true });
    const modulePath = resolve(tmpDir, 'auth.uni');
    const appPath = resolve(tmpDir, 'app.uni');

    // Test 1: Simple import
    await fs.writeFile(modulePath, `unistack module "Auth" version 1.0 {
        py-logic:
            def login(email, password): return "ok";
        routes:
            POST /auth/login { return py:login(email, password); }
    }`, 'utf8');

    await fs.writeFile(appPath, `unistack app "MyApp" version 1.0 {
        imports: from "./auth.uni" use Auth;
        config: port=3000;
    }`, 'utf8');

    await cmdBuild(tmpDir);
    const serverTs = await fs.readFile(resolve(tmpDir, 'dist', 'server.cjs'), 'utf8');
    assert(serverTs.includes("app.post('/auth/login'"), 'Module route should be present in final build');
    console.log('english: module import test passed | french: test import de module réussi');

    // Test 2: Route conflict
    await fs.writeFile(appPath, `unistack app "MyApp" version 1.0 {
        imports: from "./auth.uni" use Auth;
        routes:
            POST /auth/login { return py:app_login(); }
    }`, 'utf8');
    
    await assert.rejects(
        async () => { await cmdBuild(tmpDir); },
        (err) => {
            assert(err.message.includes('route conflict: POST /auth/login'), 'Should detect route conflict');
            return true;
        },
        'Route conflict error was not thrown'
    );
    console.log('english: module conflict test passed | french: test conflit de module réussi');

    // Test 3: Circular import
    const moduleBPath = resolve(tmpDir, 'b.uni');
    await fs.writeFile(modulePath, `unistack module "A" version 1.0 { imports: from "./b.uni" use B; }`, 'utf8');
    await fs.writeFile(moduleBPath, `unistack module "B" version 1.0 { imports: from "./auth.uni" use A; }`, 'utf8');
...
async function testDbMigrations() {
    const tmpDir = resolve('src', 'tests', 'tmp-migrations');
    await fs.mkdir(tmpDir, { recursive: true });
    const dbPath = resolve(tmpDir, 'test.db');
    const appPath = resolve(tmpDir, 'app.uni');

    const src = `unistack app "MigrateApp" version 1.0 {
        db:
          table users {
    ...
    // Mock runtime to use a test DB
    process.env.DATABASE_URL = `sqlite:${dbPath}`;
    await cmdBuild(tmpDir);

    // Check if schema.json was created
    const schemaJson = JSON.parse(await fs.readFile(resolve(tmpDir, 'dist', 'schema.json'), 'utf8'));
    assert.strictEqual(schemaJson.tables[0].name, 'users', 'Schema JSON should contain users table');

    // TODO: Need a way to run the generated server code to actually test the migration logic.
    // This would require starting the server from the test.
    console.log('english: DB migration build test passed | french: test build de migration DB réussi');
}
/**
...
async function testImports() {
    ...
    await fs.writeFile(child, `unistack app "C" version 1.0 {
  py-logic:
    def title():
      return "Imported"
}`, 'utf8');
    await fs.writeFile(parent, `unistack app "P" version 1.0 {
  imports: "child.uni";
  html-ui:
    <h1>{py:title()}</h1>;
}`, 'utf8');
...
function testAdvancedImportsParsing() {
  ...
  imports: {foo, bar as baz} from "mod.uni", "other.uni" as util, * as all from "lib.uni";
    ...
    const imports = ast.sections.find(s => s.kind === 'imports');
...
async function testValidationGenerator() {
    ...
    // remove leading export to allow eval in test environment
...
async function testUniPack() {
    ...
    await fs.writeFile(resolve(pkgSrc, 'unistack.package.json'), JSON.stringify({ name: tmpName, version: '1.0.0', main: 'app.uni' }, null, 2), 'utf8');
...
async function runAll() {
    ...
    await testMiddlewareAndWsBuild();
    await testFileUploadBuild();
