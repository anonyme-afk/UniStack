...
export function parseUniFile(source: string, fileName: string): UniFile {
  ...
  return {
    type: 'app', // assuming app for now as per Wy75.ts/ast.ts structure
    name: header.name,
...
function collectSections(lines: string[]): RawSection[] {
  ...
  const startRegex = /^\s*(imports|config|state|components|middleware|ws-routes|html-ui|css|style|ffi|py-logic|js-events|routes|db)\s*:/;
...
function parseImportsSection(lines: string[]): { kind: 'imports'; entries: any[] } {
...
function parseWsRoutesSection(lines: string[]): WsRoutesSection {
  ...
  for (const raw of defs) {
    ...
    for (const clause of clauses) {
      ...
      if (key === 'on:connect') route.onConnect = ref as any;
      else if (key === 'on:message') route.onMessage = ref as any;
      else if (key === 'on:disconnect') route.onDisconnect = ref as any;
...
function parseRouteStmt(line: string): RouteStmt {
  ...
  if (stripped.startsWith('file:')) {
    ...
    const stmt: RouteFileStmt = { kind: 'file', field, accept, maxSizeBytes: parseSizeBytes(maxSize) };
    return stmt;
  }
  throw new Error(
...
function parseExpr(text: string): any {
  if (text === 'true' || text === 'false') {
    return { kind: 'boolean', value: text === 'true' };
  }
  if (/^[0-9]+(\.[0-9]+)?$/.test(text)) {
