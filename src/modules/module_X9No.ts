...
async function testGatherImportsHelper() {
  ...
  await fs.writeFile(a, `unistack app "A" version 1.0 { imports: "b.uni"; }`, 'utf8');
  await fs.writeFile(b, `unistack app "B" version 1.0 { imports: "c.uni"; }`, 'utf8');
...
function testValidationGenerator() {
  const { AdvancedCodeGenerator } = require('../transpiler/advanced.js');
  ...
  const validateFn = eval(gen.generateValidation() + '\nvalidate');
