...
function collectSections(lines: string[]): RawSection[] {
  ...
  const startRegex = /^\s*(imports|config|state|components|middleware|ws-routes|html-ui|css|style|ffi|py-logic|js-events|routes)\s*:/;
...
function parseRouteStmt(line: string): RouteStmt {
  ...
  if (stripped.startsWith('file')) {
    const payload = stripped.slice('file'.length).trim().replace(/^:/, '').trim();
    const cfg = parseKeyValueTail(payload);
    const stmt: RouteFileStmt = {
      kind: 'file',
      accept: cfg.accept ? splitPipeList(cfg.accept) : ['*/*'],
      maxSizeBytes: cfg.maxSize ? parseSizeBytes(cfg.maxSize) : 5 * 1024 * 1024,
      field: cfg.field || 'file',
    };
    return stmt;
  }
  throw new Error(
