...
async function emitGeneratedFiles(ir: CompilationIR, generatedDir: string, devMode: boolean = false): Promise<void> {
  ...
  // if the Uni file declared environment variables, write an example file at
  // the project root. CLI also writes one, but builds invoked directly need it
  // for tooling/tests.
  if (ir.backend.env && ir.backend.env.length) {
    const exampleLines: string[] = [];
    for (const v of ir.backend.env) {
      let val = '';
      if (v.default !== undefined) {
        if (v.default.kind === 'string') val = v.default.value;
        else val = String(v.default.value);
      }
      exampleLines.push(`${v.name}=${val}`);
    }
    try {
      await fs.writeFile(resolve(outDir, '..', '.env.example'), exampleLines.join('\n'), 'utf8');
    } catch {}
  }
}
