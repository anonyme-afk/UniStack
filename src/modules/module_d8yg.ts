...
export class TypeInference {
  ...
  inferType(value: any): Type {
    ...
    if (Array.isArray(value)) {
      const itemType = value.length > 0 ? this.inferType(value[0]) : { kind: 'primitive' as const, name: 'unknown' };
      return {
        kind: 'primitive' as const,
        name: 'array',
        parameters: [itemType] as Type[],
      };
    }
...
export const typeUtils = {
  ...
  isGeneric(type: Type): boolean {
    return type.kind === 'generic' || (type.parameters != null && type.parameters.length > 0);
  },
