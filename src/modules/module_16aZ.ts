...
// global handlers to aid debugging
...
// CLI code is heavy and may require optional deps; tests will call buildUniStack directly
...
function testEnvParsing() {
  ...
  env {
    ...
    PORT: optional number default=8080;
    DEBUG: optional boolean default=false;
  }
}
`;
  const ast = parseUniFile(src, 'test.uni');
  const env = ast.sections.find(s => s.kind === 'env') as any;
  assert(env, 'env section must be parsed');
  assert(env.vars.length === 3, 'three env entries');
  assert(env.vars[0].name === 'DATABASE_URL' && env.vars[0].required === true);
  assert(env.vars[1].name === 'PORT' && env.vars[1].default === 8080);
  assert(env.vars[2].name === 'DEBUG' && env.vars[2].default === false);
  console.log('english: env parsing test passed | french: test parsing env réussi');
}
...
async function testTranspilerBuild() {
  ...
  // prepare temporary project directory for CLI build
  const project = resolve('src', 'tests', 'tmp');
  await fs.rm(project, { recursive: true, force: true });
  await fs.mkdir(project, { recursive: true });
  const entry = resolve(project, 'app.uni');
  ...
  await fs.writeFile(resolve(project, 'unistack.config.json'), JSON.stringify(cfg, null, 2), 'utf8');

  // run transpiler directly which also generates .env.example via build code
  const ir = await buildUniStack({ entryPath: entry, generatedDir: resolve(project, cfg.generatedDir) });
  // make sure ir exists for later assertions
  assert(ir && ir.backend, 'buildUniStack should return IR');

  const serverTs = await fs.readFile(resolve(project, cfg.generatedDir, 'app.server.ts'), 'utf8');
  const clientTs = await fs.readFile(resolve(project, cfg.generatedDir, 'app.client.ts'), 'utf8');
...
function testValidationGenerator() {
  const gen = new AdvancedCodeGenerator();
  const code = gen.generateValidation();
  assert(code.includes('export function validate'), 'should declare validate');
  assert(code.includes('rule.min') && code.includes('rule.max'), 'should emit min/max checks');
  assert(code.includes("case 'object'") && code.includes("case 'array'"), 'should support object/array');
