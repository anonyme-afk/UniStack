...
function buildBackendIR(ast: UniFile): BackendIR {
  ...
  const routes = routeSections.flatMap(section => section.routes).map(r => {
    ...
    const validators = r.body
      .filter((stmt): stmt is RouteValidateStmt => stmt.kind === 'validate')
    ...
    return {
      ...
      validators,
      fileUpload: fileStmt
        ? { accept: fileStmt.accept, maxSizeBytes: fileStmt.maxSizeBytes, field: fileStmt.field }
        : undefined,
  ...
  return { routes, wsRoutes, middleware, env: envVars };
}
...
function generateServerTs(ir: CompilationIR): string {
  ...
  const middlewareCfg = JSON.stringify(ir.backend.middleware);
...
<html lang="en">
  ...
  const routeLines = ir.backend.routes
    .map(route => {
      ...
      const uploadMw = route.fileUpload
        ? `express.raw({ type: '*/*', limit: '${Math.ceil(route.fileUpload.maxSizeBytes / (1024 * 1024))}mb' }), `
        : '';
      const uploadCode = route.fileUpload
        ? emitUploadGuard(route.fileUpload.accept, route.fileUpload.maxSizeBytes, route.fileUpload.field)
        : '';
      return `app.${method}('${route.path}', ${uploadMw}async (req, res) => withSandbox(req, res, '${route.method} ${route.path}', async () => {
${uploadCode}${statusLine}${validationCode}${handlerCode}
}));`;
    })
    .join('\n\n');
...
export function createServer(runtime: UniRuntime) {
  ...
  const middleware = ${middlewareCfg};
...
function emitRouteHandlerCall(handler: LangRef): string {
  if (handler.lang === 'sql') {
    const paramsExpr = handler.params.map(arg => emitArgExpr(arg)).join(', ');
...
// helper inserted for file upload guard generation
function emitUploadGuard(accept: string[], maxSizeBytes: number, field: string): string {
  const patterns = accept.map(a => `/${a.replace(/\*/g, '.*')}/`).join(' || ');
  return `
  if (!req.headers['content-type'] || !(${patterns}.test(req.headers['content-type']))) {
    res.status(400).json({ error: 'invalid content type' });
    return;
  }
  if (req.headers['content-length'] && Number(req.headers['content-length']) > ${maxSizeBytes}) {
