...
async function main(): Promise<void> {
  ...
  try {
    if (cmd === 'init') {
      ...
      console.log(
        'english: Usage: unistack <init|build|dev|fmt|lint|templates> [--watch] [--parser antlr|manual] [--port 3000] [--file path] [--template name]. ' +
          'french: Utilisation : unistack <init|build|dev|fmt|lint|templates> [--watch] [--parser antlr|manual] [--port 3000] [--file path] [--template name].',
      );
      process.exitCode = 1;
    }
  } catch (err) {
