...
async function testTranspilerBuild() {
  ...
  // run CLI build which also generates .env.example
  await cmdBuild(project);
  ...
  assert(serverTs.includes('missing required environment variable: SECRET'), 'server.ts should emit env validation for required var');
...
runAll()
  ...
  it('parses env section declarations', () => {
    const uni = `
    unistack app "envtest" version 0.0.1 {
      env {
        DATABASE_URL: required string;
        PORT: optional number default=8080;
        DEBUG: optional boolean default=false;
    ...
    `;
    const out = parseUniFile(uni);
    const env = out.sections.find(s => s.kind === 'env') as EnvSection;
