...
import { buildUniStack, CompilationIR } from './transpiler/index.js';
...
async function cmdBuild(cwd: string, parserMode?: string): Promise<CompilationIR | void> {
  if (parserMode) {
...
async function cmdDev(cwd: string, parserMode?: string, portArg?: string): Promise<void> {
  // load dotenv if available so env vars from .env are picked up
  try {
